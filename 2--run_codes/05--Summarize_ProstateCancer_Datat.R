...
base::source(file = "2--run_codes/NOT_RUN_RCODES/MethodsAndPackages.R")
...
base::load(file = "misc/app_misc_results/RealDtPoolMetrics")
...
ParamsMRL <- Top3Methods %>%
  dplyr::select(method, contains(c("mean"))) %>%
  tibble::add_row(
...
SampleMRLifeTable <- dplyr::mutate(SampleMRLifeTable, across(tidyselect::all_of(nms), ~ ((.x / Integrated - 1) * 100) %>%
...
lftab <- SampleMRLifeTable %>%
  dplyr::relocate(c("MLE", "Moments", "Regression"), .after = "Integrated") %>% 
  ...
  add_header_above(c(" " = 2,  "Classics" = 3, "MCMC" = 3))
