...
transformed data{
    real<lower=0> mu_lnorm_beta = (log(mu_shape / sqrt( (var_shape / square(mu_shape) ) +1 ) ) )* fctor;
    real sigma_lnorm_beta = (sqrt( log( (var_shape / square(mu_shape) ) +1 )  ) )* fctor;
    real <lower = 0> shape_inv_gamma = ((square(mu_scale) / var_scale ) +2 )* fctor;
    real < lower = 0> scale_inv_gamma = (mu_scale + (pow(mu_scale,3) / var_scale) )* fctor;
}
parameters{
...
model{
    ...
    shape_inv_gamma ~ lognormal(m, v);
    scale_inv_gamma ~ lognormal(m, v);
    beta ~ lognormal(mu_lnorm_beta, sigma_lnorm_beta);  // parameter priors
    alpha ~ inv_gamma(shape_inv_gamma, scale_inv_gamma);
