...
transformed data{
    ...
    real<lower=0> sigma_scale = sqrt(var_scale)* fctor;
}
parameters{
...
model{
    mu_lnorm_beta ~ normal(m,v);   // hyper priors shape parameters     
    sigma_lnorm_beta ~ lognormal(m, v); 
    mu_scale_param ~ normal(m, v);   // half normal hyper -parameters       
