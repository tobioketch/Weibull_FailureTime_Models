...
transformed data{
    ...
    real <lower = 0> lambda_shape = inv(mu_shape)* fctor ; // for shape parameter
...
model{
    lambda_shape ~ lognormal(m, v);   // hyper priors
    mu_scale_param ~ normal(m, v);           
    sigma_scale_param ~ lognormal(m, v);  
    beta ~ exponential(lambda_shape); // parameter priors
    alpha ~ normal(mu_scale_param, sigma_scale_param); // Half normal priors by constrain on alpha
